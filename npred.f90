program main
  ...
  real*8  freqL1(4),tranL1(4,4),tranL2(16,4),tranL3(64,4),tranL4(256,4),Pdd(500,11),Pd(500)
  real*8  freqN4(64,4),tranN4((147-4)*256,4),freqN1(147,4),tranN1(584,4)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  ind=0
  open(1,file='./mnase/freqL.txt')  !##### freqL1
  read(1,*) freqL1; close(1)
  open(1,file='./mnase/tranL.txt')  !##### tranL1
  do i=1,4; read(1,*) tranL1(i,:); end do; close(1)
  open(1,file='./mnase/tranL2.txt') !##### tranL2
  do i=1,16; read(1,*) tranL2(i,:); end do; close(1)
  open(1,file='./mnase/tranL3.txt') !##### tranL3
  do i=1,64; read(1,*) tranL3(i,:); end do; close(1)
  open(1,file='./mnase/tranL4.txt') !##### tranL4
  do i=1,256; read(1,*) tranL4(i,:); end do; close(1)

  open(1,file='./mnase/147freqN.txt') !freqN1
  do i=1,147; read(1,*) freqN1(i,:); end do
  close(1)
  open(2,file='./mnase/147tranN.txt') !tranN1
  do i=1,584; read(2,*) tranN1(i,:); end do
  close(2)

  open(1,file='./mnase/146-149freqN4.txt') !##### freqN4
  do i=1,64; read(1,*) freqN4(i,:); end do; close(1)
  open(1,file='./mnase/146-149tranN4.txt') !##### tranN4
  ...
  if(order==1) then
    call vtbfb(lfn,trim(fileName),freqL1,tranL1,freqN1,tranN1,mlL,rep,species,Pdd,ind)
  else if(order==4) then
    call vtbfbNL4(lfn,trim(fileName),freqL1,tranL1,tranL2,tranL3,tranL4,freqN4,tranN4,mlL,rep,species,Pdd, ind)
  end if  
end
...
6 if(l==1) then
  ...
  lfn1=lfn-i
  out1(1:lfn1)=filename(i+1:lfn); out1(lfn1+1:lfn1+16)='_P1.txt'
...
6 if(l==1) then
  ...
  do i=lfn,1,-1; if(filename(i:i)=='/'.or.filename(i:i)=='\') exit; end do
  lfn1=lfn-i
  out1(1:lfn1)=filename(i+1:lfn); out1(lfn1+1:lfn1+16)='_P4.txt'
  open(1,file=out1(1:lfn1+16),status='replace'); temp=-0.05; t=0; tpc='NA'
  write(1,*) 'Position  P-start Occup  N/L  Affinity'
  do i=1,nrcv(1)-1; write(1,'(i9,2x,f5.2,3x,f5.2,3x,i1,4x,a)') i,temp,temp,t,tpc; end do
  close(1)
